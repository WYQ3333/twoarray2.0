...
double findMedianSortedArrays1(int* nums1, int nums1Size, int* nums2, int nums2Size)
{
	if (nums1Size == 0 || nums2Size==0)
	{
		if (nums1Size == 0)
		{
			return (nums2Size % 2 == 0) ? (nums2[nums2Size / 2] + nums2[nums2Size]) / 2.0 : nums2[nums2Size] / 2.0;
		}
		if (nums2Size == 0)
		{
			return (nums1Size % 2 == 0) ? (nums1[nums2Size / 2] + nums1[nums2Size]) / 2.0 : nums1[nums2Size] / 2.0;
		}
	}
	else
	{
